...
class PageManager {
  ...
  final allSongSm = Get.put(AllSongsScreenModel());
  final homeSM = Get.put(HomeScreenModel());
  int maxLength = 4;
  listenToChangesInSong() {
    audioHandler.mediaItem.listen((mediaItem) {
      currentSongNotifier.value = mediaItem;
      allSongSm.currentId.value = mediaItem!.id;
      updateSkipButton();

      homeSM.recentlyPlayed.addAll(allSongSm.allSongs.where((item) {
        final itemId = item["id"];

        bool notExists = homeSM.recentlyPlayed
            .every((recentItem) => recentItem["id"] != itemId);

        return itemId == allSongSm.currentId.value && notExists;
      }));
      if (homeSM.recentlyPlayed.length > maxLength) {
        homeSM.recentlyPlayed.remove(homeSM.recentlyPlayed[0]);
      }
    });
  }
